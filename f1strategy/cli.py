...
def cmd_predict(args) -> None:
    ...
    pred = predict_strategy(args.track, args.year, use_practice=args.practice,
                            use_sc=not args.no_sc, seasons_back=args.seasons)
...
def cmd_calibrate(args) -> None:
    ...
    path = calibrate.save_params(params, sc_on=sc_on, meta={
...
def cmd_backtest(args) -> None:
    ...
    config = SimConfig(use_sc=sc_on)
    cases = [c for c in build_dataset(use_practice=args.practice) if c.usable]
    params = calibrate.load_params(sc_on=sc_on)
    backtest.report(cases, params, config, f"backtest (SC {'on' if sc_on else 'off'})")
